// src/components/AdminTeacherList.jsx
import React, { useState, useEffect } from "react";
import { useTheme } from "../context/ThemeContext";
import { FaUser, FaEnvelope, FaUniversity, FaSpinner, FaTrash } from "react-icons/fa";
...
const AdminTeacherList = () => {
    ...
    useEffect(() => {
        const fetchTeachers = async () => {
            ...
            const response = await fetch(
                `${import.meta.env.VITE_BASE_URL}/auth/list`
            );
  ...
  const handleDelete = async (email) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL}/auth/delete/${email}`,
        { method: "DELETE" }
      );
      ...
      >
        ...
        >
          <h2 className="text-2xl font-bold flex items-center gap-2">
            <FaUser className="text-blue-500" /> Teacher Management
          </h2>
          <p
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div
                        className={`flex-shrink-0 h-10 w-10 rounded-full flex items-center justify-center transition-colors duration-300 ${
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-center">
                    <button
                      ...
                      className="py-2 px-4 cursor-pointer flex justify-center items-center gap-2 rounded-md bg-red-100 hover:bg-red-200 text-red-600 transition-colors"
                    >
                      <FaTrash /> Delete
                    </button>
                  </td>
