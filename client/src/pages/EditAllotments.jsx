...
const EditAllotments = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`${BASE_URL}/allotments/edit`, {
        method: "PUT",
      ...
      if (!res.ok) throw new Error("Failed to edit allotments");

      setMessage("✅ Allotments update successfully");
      setSelectedCourses([]);
      setSelectedTeacher("");
    } catch (err) {
      setMessage("❌ Error edit allotments.");
      console.error(err);
    } finally {
      setLoading(false);
    }
